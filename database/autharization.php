...
class AuthClass
{
    ...
    public function setCookie($login, $password)
    {
        ...
        if (!$update_password_cookie_token) {
            ...
            header("Location: /sakura/pages/menus/auth.php");

            //Останавливаем скрипт
            exit();
        }

        setcookie("password_cookie_token", $password_cookie_token, time() + 36000, '/sakura');
    }
    //удаление куки
    public function deleteCookie()
    {
        if (isset($_COOKIE["password_cookie_token"])) {
            ...
            setcookie("password_cookie_token", null, time() - 36000, '/sakura');
    ...
    public function IsCookie()
    {
        if (isset($_COOKIE["password_cookie_token"])) {
            ...
            if (!empty($array['password_cookie_token'])) {
                header("Location: /sakura/pages/profile/orders.php");
            }
        }
    }
    ...
    {
        if (!isset($_COOKIE["password_cookie_token"])) {
            return 'onclick="window.location.href=`/sakura/pages/menus/auth.php`"';
        }
    }
}
...
class SetVal
{
    ...
    public function SendMail($fio, $array_id, $email)
    {
        global $pdo;

        $message = "Добрый день, $fio!\n";
        $message .= "<br>";
        $message .= "Ваш заказ обрабатывается и вскоре мы с вами свяжемся.";
        $message .= "Ваш заказ: ";
        foreach ($array_id as $key) {
            $good = $pdo->query("SELECT * FROM public.goods WHERE id = $key")->fetch(PDO::FETCH_ASSOC);
            $message .= "    - " . $good['description'] . " " . $good['price'];
        }
        $message .= "<br><br>";
        $message .= "\n - - - - - - - - - - - - - \n";
        $message .= "<br><br>";
        ...
        $message .= "<br>";
        $message .= "E-mail: sakura-flowwwers@mail.com\n";
        $message .= "<br>";
